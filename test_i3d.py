...
def run(init_lr=0.1,
    ...
    # Tránh chia cho 0
    top1_acc_class = np.divide(top1_tp, top1_tp + top1_fp, out=np.zeros_like(top1_tp, dtype=float),
                               where=(top1_tp + top1_fp) != 0)
    top5_acc_class = np.divide(top5_tp, top5_tp + top5_fp, out=np.zeros_like(top5_tp, dtype=float),
                               where=(top5_tp + top5_fp) != 0)
    top10_acc_class = np.divide(top10_tp, top10_tp + top10_fp, out=np.zeros_like(top10_tp, dtype=float),
                                where=(top10_tp + top10_fp) != 0)

    top1_per_class = np.mean(top1_acc_class)
    top5_per_class = np.mean(top5_acc_class)
    top10_per_class = np.mean(top10_acc_class)
...
if __name__ == '__main__':
    ...
    # Resolve train_split relative to this script so the file is found
    # regardless of the current working directory when running the script.
    script_dir = os.path.dirname(os.path.realpath(__file__))
    train_split = os.path.join(script_dir, 'preprocess', 'nslt_{}.json'.format(num_classes))
