...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    ...
    if not vidcap.isOpened():
        print("[WARN] Cannot open video:", video_path)
        # trả về 1 frame đen để không lỗi
        dummy = np.zeros((resize[1], resize[0], 3), dtype=np.float32)
        return np.asarray([dummy], dtype=np.float32)

    total_frames = int(vidcap.get(cv2.CAP_PROP_FRAME_COUNT))

    # nếu start nằm ngoài số frame thì cũng coi như video hỏng
    if total_frames <= start:
        print("[WARN] Not enough frames:", video_path, "total:", total_frames, "start:", start)
        dummy = np.zeros((resize[1], resize[0], 3), dtype=np.float32)
        return np.asarray([dummy], dtype=np.float32)

    vidcap.set(cv2.CAP_PROP_POS_FRAMES, start)

    for offset in range(min(num, max(0, total_frames - start))):
        success, img = vidcap.read()
        if (not success) or (img is None):
            print("[WARN] Failed to read frame", start + offset, "from", video_path)
            break

        h, w, c = img.shape  # cv2: (H, W, C)

        # scale nhỏ nhất lên >= 226 (giống code cũ)
        if h < 226 or w < 226:
            d = 226. - min(h, w)
            sc = 1 + d / min(h, w)
            img = cv2.resize(img, dsize=(0, 0), fx=sc, fy=sc)

        # resize về max 256 (giống code cũ)
        if h > 256 or w > 256:
    ...
    # nếu không đọc được khung nào thì trả về 1 frame đen
    if len(frames) == 0:
        print("[WARN] No readable frame in:", video_path)
        dummy = np.zeros((resize[1], resize[0], 3), dtype=np.float32)
        return np.asarray([dummy], dtype=np.float32)
